$fn = 50;
use <gridfinity_base_tile_raw_47ed795bf3cbfe2f.scad>


difference() {
	union() {
		translate(v = [-42.0000000000, -105.0000000000, 0]) {
			cube(size = [87, 210, 1.5000000000]);
		}
		translate(v = [-42.0000000000, 103.0000000000, 0]) {
			cube(size = [87, 2, 15]);
		}
		translate(v = [43.0000000000, -105.0000000000, 0]) {
			cube(size = [2, 210, 15]);
		}
		translate(v = [-42.0000000000, -105.0000000000, 0]) {
			cube(size = [2, 210, 15]);
		}
		#translate(v = [-21.0000000000, -84.0000000000, -4.6000000000]) {
			gridfinity_base_tile_raw(distancex = 0, distancey = 0, fitx = 0, fity = 0);
		}
		#translate(v = [-21.0000000000, -42.0000000000, -4.6000000000]) {
			gridfinity_base_tile_raw(distancex = 0, distancey = 0, fitx = 0, fity = 0);
		}
		#translate(v = [-21.0000000000, 0.0000000000, -4.6000000000]) {
			gridfinity_base_tile_raw(distancex = 0, distancey = 0, fitx = 0, fity = 0);
		}
		#translate(v = [-21.0000000000, 42.0000000000, -4.6000000000]) {
			gridfinity_base_tile_raw(distancex = 0, distancey = 0, fitx = 0, fity = 0);
		}
		#translate(v = [-21.0000000000, 84.0000000000, -4.6000000000]) {
			gridfinity_base_tile_raw(distancex = 0, distancey = 0, fitx = 0, fity = 0);
		}
		#translate(v = [21.0000000000, -84.0000000000, -4.6000000000]) {
			gridfinity_base_tile_raw(distancex = 0, distancey = 0, fitx = 0, fity = 0);
		}
		#translate(v = [21.0000000000, -42.0000000000, -4.6000000000]) {
			gridfinity_base_tile_raw(distancex = 0, distancey = 0, fitx = 0, fity = 0);
		}
		#translate(v = [21.0000000000, 0.0000000000, -4.6000000000]) {
			gridfinity_base_tile_raw(distancex = 0, distancey = 0, fitx = 0, fity = 0);
		}
		#translate(v = [21.0000000000, 42.0000000000, -4.6000000000]) {
			gridfinity_base_tile_raw(distancex = 0, distancey = 0, fitx = 0, fity = 0);
		}
		#translate(v = [21.0000000000, 84.0000000000, -4.6000000000]) {
			gridfinity_base_tile_raw(distancex = 0, distancey = 0, fitx = 0, fity = 0);
		}
	}
	union();
}